...
- **The full interface/website is only fully visible after logging in.** Logged-out users only get the public home page; signed-in (admin) users unlock the rest (e.g. the `/admin` dashboard and authenticated booking actions). To exercise the complete UI, log in with the admin test account. Its credentials are provided via Secrets (env vars `FRC_ADMIN_EMAIL` / `FRC_ADMIN_PASSWORD`) — type them into the login form during manual testing; the app does not read login creds from env. Login still requires the `VITE_FIREBASE_*` config to be set so the client can reach Firebase Auth.
