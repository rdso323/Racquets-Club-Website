...
- **Auth is passwordless email link for `firstname.lastname@duke.edu`** (see `src/contexts/AuthContext.tsx`); admin access is limited to a hardcoded allowlist of emails. Home/Cabinet/Help are public; booking requires sign-in. End-to-end auth/booking flows need a real Firebase project with Email link enabled and a Duke inbox, so they are not fully testable with placeholder config alone.
